...
SELECT bedrooms,AVG(price) avg_price
FROM Listings 
...
SELECT zipcode,ROUND(AVG(price),2) avg_price,ROUND(AVG(weekly_price),2) weekly_price,ROUND(AVG(monthly_price),2) monthly_price
FROM Listings 
...
SELECT property_type,ROUND(AVG(bedrooms),0) number_of_rooms,ROUND(AVG(bathrooms),0) number_of_bathrooms
FROM Listings 
...
SELECT neighbourhood,AVG(price) avg_price,COUNT(property_type) num_of_properties
FROM Listings 
...
SELECT property_type, COUNT(*) number
FROM Listings 
...
SELECT property_type,AVG(price) average_price
FROM Listings
WHERE property_type IS NOT NULL
GROUP BY property_type
...
SELECT property_type,AVG(cleaning_fee) cleaning_fee
FROM Listings 
